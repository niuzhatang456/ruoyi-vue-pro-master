<template>
  <PageShell
    title="图片/文件识别录入"
    description="上传图片、PDF、Word 等文件，后端生成导入记录并返回自动识别类型。"
  >
    <el-upload
      class="upload-area"
      drag
    ...
    >
      ...
      <template #tip>
        <div class="el-upload__tip">
          OCR 识别录入使用 file 字段上传，成功后会写入最近导入记录。
        </div>
      </template>
    </el-upload>
...
import { getParsedData, uploadOcrImportFile } from '@/api/jijian/import'
...
const handleUploadChange = async (file: UploadFile) => {
  ...
  loading.value = true
  try {
    const record = await uploadOcrImportFile(file.raw)
