...
$previewDir = 'D:\VScode\data\ruoyi-vue-pro-master\outputs\ppt_text_edit\preview_balanced'

if (-not (Test-Path -LiteralPath $backupPath)) {
  throw '找不到原始备份：$backupPath'
}
Copy-Item -LiteralPath $backupPath -Destination $pptPath -Force
...
function Get-TextItems {
  ...
  foreach ($shape in $Shapes) {
    try {
      if ($shape.Type -eq 6) {
        $children = Get-TextItems $shape.GroupItems
        foreach ($child in $children) { $items.Add($child) }
      } elseif ($shape.Visible -and $shape.HasTextFrame -and $shape.TextFrame.HasText) {
...
function Unique-ByBox {
  param([object[]]$Items)
  $seen = @{}
  $out = New-Object System.Collections.Generic.List[object]
  foreach ($item in $Items) {
    $key = '{0}:{1}:{2}:{3}' -f ([math]::Round($item.Left / 8)), ([math]::Round($item.Top / 8)), ([math]::Round($item.Width / 8)), ([math]::Round($item.Height / 8))
...
function Apply-Text {
  param($Item, [string[]]$Lines, [double]$Size, [bool]$Bold = $false)
  $text = ($Lines -join [Environment]::NewLine)
  $tr = $Item.Shape.TextFrame.TextRange
  $tr.Text = $text
  $tr.Font.Size = $Size
  $tr.Font.Bold = $(if ($Bold) { -1 } else { 0 })
  try { $Item.Shape.TextFrame.WordWrap = -1 } catch {}
  try { $Item.Shape.TextFrame2.AutoSize = 2 } catch {}
}

function Clear-Text {
  param($Item)
  try { $Item.Shape.TextFrame.TextRange.Text = '' } catch {}
}

function Split-Lines {
  param([string[]]$Lines, [int]$Buckets)
  $result = @()
  if ($Buckets -le 1) { return ,@($Lines) }
  $per = [math]::Ceiling($Lines.Count / $Buckets)
  for ($i = 0; $i -lt $Lines.Count; $i += $per) {
    $take = [Math]::Min($per, $Lines.Count - $i)
    $result += ,@($Lines[$i..($i + $take - 1)])
  }
  return $result
}
...
try {
  ...
  for ($i = 1; $i -le 28; $i++) {
    ...
    $entry = $outline[$i - 1]
    $items = @(Unique-ByBox @(Get-TextItems $slide.Shapes))
    $assigned = New-Object System.Collections.Generic.HashSet[int]

    function Mark($item, [string[]]$lines, [double]$size, [bool]$bold = $false) {
      if ($null -eq $item) { return }
      Apply-Text $item $lines $size $bold
      [void]$assigned.Add([int]$item.Id)
    }

    if ($i -eq 1) {
      $main = $items | Where-Object { $_.Area -gt 20000 } | Sort-Object Area -Descending | Select-Object -First 1
      Mark $main @($entry.title, $entry.body[0]) 30 $true
      $bottom = @($items | Where-Object { $_.Top -gt 330 -and $_.Id -ne $main.Id } | Sort-Object Left)
      if ($bottom.Count -ge 3) {
        Mark $bottom[0] @('姓名：', '专业：') 17 $false
        Mark $bottom[1] @('指导老师：', '学校：') 17 $false
        Mark $bottom[2] @('答辩时间：') 17 $false
      }
    } elseif ($i -eq 2) {
      $titleShape = $items | Where-Object { $_.Text -eq '目录' } | Select-Object -First 1
      Mark $titleShape @($entry.title) 34 $true
      ...
      for ($m = 0; $m -lt [Math]::Min($menuShapes.Count, $entry.body.Count); $m++) {
        Mark $menuShapes[$m] @($entry.body[$m]) 18 $true
      }
    } else {
      $navShapes = @($items | Where-Object { $_.Top -lt 90 -and $_.Left -gt 320 -and $_.Width -gt 45 } | Sort-Object Left)
      for ($n = 0; $n -lt [Math]::Min($navShapes.Count, $nav.Count); $n++) {
        Mark $navShapes[$n] @($nav[$n]) 7.5 $false
      }

      $pageShape = $items | Where-Object { $_.Text -like 'Page*' } | Select-Object -First 1
      if ($pageShape) { Mark $pageShape @(('Page ' + $i)) 10 $false }

      $titleCandidates = @($items | Where-Object { $_.Left -lt 330 -and $_.Top -lt 105 -and $_.Width -gt 120 } | Sort-Object Top, Left)
      ...
      Mark $titleItem @($entry.title) 24 $true

      $dividerSlides = @(3, 9, 14, 19, 24)
      if ($dividerSlides -contains $i) {
        $rightBox = $items | Where-Object {
          -not $assigned.Contains([int]$_.Id) -and $_.Left -gt 620 -and $_.Top -gt 165 -and $_.Area -gt 1200
        } | Sort-Object Area -Descending | Select-Object -First 1
        $keywordBox = $items | Where-Object {
          -not $assigned.Contains([int]$_.Id) -and $_.Left -lt 620 -and $_.Top -gt 250 -and $_.Width -gt 260 -and $_.Area -gt 1000
        } | Sort-Object Area -Descending | Select-Object -First 1
        if ($rightBox) {
          $rightLines = $entry.body | Select-Object -First 4
          Mark $rightBox $rightLines 15 $false
        }
        if ($keywordBox) {
          $keywords = switch ($i) {
            3 { @('技术推动', '虚实结合', '沉浸体验', '空间塑造') }
            9 { @('透视', '投影', '光影', '纵深') }
            14 { @('场景转换', '时间流动', '心理变化', '叙事连贯') }
            19 { @('失重动线', '多维空间', '身体感知', '空间关系') }
            24 { @('UE 构建', '非重力建筑', '天幕纱幕', '虚实边界') }
          }
          Mark $keywordBox $keywords 15 $false
        }
        foreach ($item in $items) {
          if ($assigned.Contains([int]$item.Id)) { continue }
          if ($item.Text -match '^\d{1,2}$') { continue }
          Clear-Text $item
        }
        continue
      }

      $bodyCandidates = @($items | Where-Object {
        -not $assigned.Contains([int]$_.Id) -and $_.Top -gt 105 -and $_.Area -gt 1200 -and $_.Text -notmatch '^\d{1,2}$'
      } | Sort-Object Top, Left)

      if ($bodyCandidates.Count -eq 0) {
        $bodyCandidates = @($items | Where-Object {
          -not $assigned.Contains([int]$_.Id) -and $_.Area -gt 900 -and $_.Text -notmatch '^\d{1,2}$'
        } | Sort-Object Area -Descending)
      }

      $usable = @($bodyCandidates | Select-Object -First ([Math]::Min(4, [Math]::Max(1, $bodyCandidates.Count))))
      if ($usable.Count -gt 0) {
        $chunks = @(Split-Lines $entry.body $usable.Count)
        for ($b = 0; $b -lt [Math]::Min($usable.Count, $chunks.Count); $b++) {
          $lineCount = $chunks[$b].Count
          $size = if ($lineCount -ge 3) { 13 } elseif ($lineCount -eq 2) { 15 } else { 17 }
          Mark $usable[$b] $chunks[$b] $size $false
        }
      } else {
        Mark $titleItem @($entry.title + $entry.body) 15 $true
      }
    }
